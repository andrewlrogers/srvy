...
def generate_dataframe():
    ...
    df.ix[df.opinion > 0, 'like_dislike'] = 'Liked' #if the value in opinion is > 0 then the value in like_dislike is 'Liked'
...
def create_all_charts():
    ...
    question_bar = Bar(df.sort_values(by='opinion', ascending = False), title='Opinion reponses by question asked.', values = 'like_dislike', label = 'question', stack = 'like_dislike', ylabel = 'Number of Responses', agg = 'count', legend= 'top_right', palette = crocker_contrast)
    chart_group.append(question_bar)

    print('Creating donut chart of overall like/dislike')
    donut_chart = Donut(df, title = 'Like and Dislike distribution', label=['like_dislike'], values = 'opinion', agg='count', palette=crocker_purple)
    pie_group.append(donut_chart)

    print('Creating donut chart of question distribition')
    q_dist = Donut(df, title = 'Distribition of questions asked',label='question', hover_text='question', hover_tool = True, values = 'question', agg='count', palette=crocker_purple)
    pie_group.append(q_dist)

    print('Creating an hourly Bar Chart')
    hourly_bar = Bar(df.sort_values(by='opinion', ascending = False), title='Opinion by hour of the day', values = 'opinion', label = 'hour', stack = 'like_dislike', xlabel='Hour of the Day', ylabel = 'Number of Responses', agg = 'count', legend= 'top_right', palette = crocker_contrast)
    chart_group.append(hourly_bar)

    print('Creating score')
    total_score = Bar(df_score, title='Score by question: 0 is neutral', label = 'question', values = 'score', legend = False, ylabel = 'Score' , palette = crocker_contrast)
